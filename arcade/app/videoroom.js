// Mostly stolen from Janus Videoroom example: https://github.com/meetecho/janus-gateway
...
var myroom = parseInt( getQueryVar( 'video_room' ) );  // Demo room
var myusername = null;
var myid = null;
var mystream = null;
...
$(document).ready(function() {
  ...
  Janus.init({debug: "none", callback: function() {
	...
	$(this).attr('disabled', true).unbind('click').hide();
      // Make sure the browser supports WebRTC
      if(!Janus.isWebrtcSupported()) {
        bootbox.alert("Your browser doesn't support video streaming!");
        return;
      }
      // Create session
      janus = new Janus(
          ...
          success: function() {
		    ...
		    registerUsername();
                ...
                error: function(error) {
                  ...
                  bootbox.alert("Error connecting to server!" + error);
                },
                consentDialog: function(on) {
                  ...
                  if(on) {
                    ...
                    $.blockUI({
                      message: '<div><img src="app/images/up_arrow.png"/></div>',
                      css: {
                        border: 'none',
                        padding: '15px',
			...
			var view_only = getQueryVar( 'view_only' );
			if( !view_only ) publishOwnFeed(true); // TODO: Don't call this if only listening
                      ...
                      bootbox.alert("The gamer doesn't stream anymore!", function() {
                ...
                onlocalstream: function(stream) {
                  ...
                  if($('#myvideo').length === 0) {
                    $('#videolocal').append('<video class="rounded right" id="myvideo" width="100%" height="27%" autoplay playsinline muted="muted"/>');
                    // Add a 'mute' button
                    $('#videolocal').append('<button class="btn btn-warning btn-xs" id="mute" style="position: absolute;  left: 15px; ">🎤</button>');
                    $('#mute').click(toggleMute);
                    // Add an 'unpublish' button
                    $('#videolocal').append('<button class="btn btn-warning btn-xs" id="unpublish" style="position: absolute; right: 15px;">🎥</button>');
                  ...
                  if(sfutest.webrtcStuff.pc.iceConnectionState !== "completed" &&
                    ...
                    $("#videolocal").parent().parent().block({
                      message: '<b>Sending video feed...</b>',
                      css: {
                        border: 'none',
                        backgroundColor: 'transparent',
                        color: 'white'
                ...
                oncleanup: function() {
                  ...
                  mystream = null;
                  $('#videolocal').html('<button id="publish" class="btn btn-primary">📹</button>');
  ...
  sfutest.createOffer(
    {
      ...
      error: function(error) {
        ...
        if (useAudio) {
          ...
          bootbox.alert("Server error... " + JSON.stringify(error));
  ...
  janus.attach(
    {
      ...
      onremotestream: function(stream) {
        ...
        if($('#remotevideo'+remoteFeed.rfindex).length === 0) {
          addButtons = true;
            // No remote video yet
	    // '<video class="rounded right" id="myvideo" width="27%" height="27%" autoplay playsinline muted="muted"/>')
          $('#videoremote'+remoteFeed.rfindex).append('<video class="rounded right" id="waitingvideo' + remoteFeed.rfindex + '" width="27%" height="27%" />');
          $('#videoremote'+remoteFeed.rfindex).append('<video class="rounded right relative hide" id="remotevideo' + remoteFeed.rfindex + '" width="27%" height="27%" autoplay playsinline/>');
        ...
        if(videoTracks === null || videoTracks === undefined || videoTracks.length === 0) {
          ...
          if($('#videoremote'+remoteFeed.rfindex + ' .no-video-container').length === 0) {
            $('#videoremote'+remoteFeed.rfindex).append(
              '<div class="no-video-container" width="27%">' +
                '<i class="fa fa-video-camera fa-5 no-video-icon"></i>' +
